...
public:
	...
	std::vector<float> return_Parameters();
};
...
std::vector<float> spline::return_Parameters()
{
	std::vector<float> list;

	for (int i = 0; i < m_a.size(); i++)
	{
		list.emplace_back( (float) m_a[i] );
		list.emplace_back( (float) m_b[i] );
		list.emplace_back( (float) m_c[i] );
	}
